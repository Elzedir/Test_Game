...
public class Player : MonoBehaviour
{
    ...
    public virtual void PlayerMove()
    {
        if (_playerActor != null && !_playerActor.ActorStates.Dead)
        {
            ...
            if (Input.GetKey(Manager_Input.Instance.KeyBindings.Keys[ActionKey.Move_Down])) y = -1;
            if (Input.GetKey(Manager_Input.Instance.KeyBindings.Keys[ActionKey.Move_Left])) x = -1;
            if (Input.GetKey(Manager_Input.Instance.KeyBindings.Keys[ActionKey.Move_Right])) x = 1;

            if (!_playerActor.ActorStates.Dodging)
            {
            ...
            Vector3 lookDirection = _playerActor.ActorStates.Dodging ? moveDirection : mouseDirection;

            transform.localScale = new Vector3(_playerActor.OriginalSize.z * Mathf.Sign(lookDirection.x), _playerActor.OriginalSize.y, _playerActor.OriginalSize.z);
